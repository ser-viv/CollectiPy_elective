...
class SpinSystem:
    ...
    def _to_pm1(self, state):
        """
        Converte lo stato interno 0/1 in ±1 per il calcolo della Hamiltoniana.
        0 → -1 (inattivo)
        1 → +1 (attivo)
        Questa conversione è necessaria perché la Hamiltoniana del paper
        usa σ ∈ {-1, +1}, non {0, 1}.
        """
    ...
    def calculate_hamiltonian(self, state):
        """
        Calcola la Hamiltoniana H = -[1/Ns * Σ Jij σi σj + Σ hi σi - hb σi]
        con σ ∈ {-1, +1} come nel paper Salahshour & Couzin.
        Lo stato interno è 0/1 (uint8), la conversione a ±1 avviene qui.
        """
        # conversione 0/1 → ±1
        state_pm1 = self._to_pm1(state.ravel())

        # termine di interazione tra spin: -J/Ns * Σ_ij Jij σi σj
        interaction = state_pm1[:, None] * state_pm1[None, :]
        H_spin_interactions = -(self.J / (self.num_spins_per_group * self.num_groups)) * np.sum(np.triu(self.J_matrix * interaction, 1))

        #eq 4 del allocentric flocking pag 11 74-81 righe 
        # aggiungere i termini hi di eq 3 h0 e sigma norma e deviazione della gaussiana
        # calcolare il vettore hi. h_b mettere a un termine costante piccolo e zero l'altra

        # termine di campo esterno: -Σ hi σi
        external_field_contribution = -np.dot(self.external_field, state_pm1)
    ...
    def average_direction_of_activity(self):
        """
        Calcola la direzione media degli spin attivi (σ = +1).
        Usa la rappresentazione 0/1 direttamente: spin attivo = 1.
        """
        ...
        #print(f"[SPIN] bump direction={math.degrees(angle_rad):.1f}° "f"heading agente={self.agent.orientation.z:.1f}°")
